...
class MessagePart(
  ...
  companion object {
    fun fromJson(json: JSONObject): MessagePart {
      val arrayPart =
          try {
            json.getJSONArray("parts")
          } catch (error: Exception) {
            null
          }
      val parts = arrayPart?.let { arrayOfNulls<MessagePart>(it.length()) }
      if (arrayPart != null) {
        for (i in 0 until arrayPart.length()) parts?.set(i, fromJson(arrayPart.getJSONObject(i)))
      }

      val arrayHeaders =
          try {
            json.getJSONArray("headers")
          } catch (error: Exception) {
            null
          }
      val headers = arrayHeaders?.let { arrayOfNulls<MessageHeader>(it.length()) }
      if (arrayHeaders != null) {
        for (i in 0 until arrayHeaders.length()) headers?.set(
            i, MessageHeader.fromJson(arrayHeaders.getJSONObject(i)))
      }

      return MessagePart(
          try {
            json.getString("partId")
          } catch (error: Exception) {
            null
          },
          try {
